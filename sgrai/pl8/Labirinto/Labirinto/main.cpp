...
{
    double base = rand() % 4 * 0.25;
    double base2 = rand() % 4 * 0.25;
    glBegin(GL_POLYGON);
        glNormal3fv(normal);
        glTexCoord2f(base,base2);
        glVertex3fv(a);
        glTexCoord2f(base + 0.25, base2);
        glVertex3fv(b);
        glTexCoord2f(base + 0.25, base2 +0.25);
        glVertex3fv(c);
        glTexCoord2f(base, base2 + 0.25);
        glVertex3fv(d);
    glEnd();
}
...
void desenhaBussola(int width, int height)  // largura e altura da janela
{
  ...
  //desenha bussola 2D
        glPushMatrix();
            ...
            glPushMatrix();
                glColor3f(0.4,0.4,1);
                strokeCenterString("S", 0, -20, 0 , 0.1); // string, x ,y ,z ,scale
                glBegin(GL_POLYGON);
                    glVertex2d(-5,0);
                    glVertex2d(5,0);
                    glVertex2d(0,-10);
