...
void poligono(GLint n, GLdouble x0, GLdouble y0, GLdouble r) {
    ...
    for (int i = 0; i < n; ++i) {
        ponto ponto = calcularPonto(angulo, r, centro);
...
void mostrador(GLdouble x0, GLdouble y0, GLdouble r) {
    ...
    for (int i = 0; i < numMinutos; ++i) {
        ...
        //Marcas de Horas:
        if (i % 5 == 0 && i % 15 != 0) {
            ponto pmin = calcularPonto(angulo, rhmin, centro);
            ponto pmax = calcularPonto(angulo, rhmax, centro);
...
void ponteiros(GLdouble x0, GLdouble y0, GLdouble r, GLint h, GLint m, GLint s) {
    ...
    ponto pmax = calcularPonto(angHora, rh, centro);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2d(centro.x, centro.y);
    glVertex2d(pmax.x, pmax.y);

    pmax = calcularPonto(angMinuto, rm, centro);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2d(centro.x, centro.y);
    glVertex2d(pmax.x, pmax.y);

    pmax = calcularPonto(angSegundo, rs, centro);
    glColor3f(0.0f, 0.0f, 0.0f);
    glVertex2d(centro.x, centro.y);
    glVertex2d(pmax.x, pmax.y);
