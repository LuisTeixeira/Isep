...
void inicia_jogo() {
    ...
    modelo.jogador1.x = LINHA_SERVICO + LARGURA_RAQUETES * 0.5;
    modelo.jogador2.x = LARGURA_CAMPO - (LINHA_SERVICO + LARGURA_RAQUETES * 0.5);
    modelo.jogador1.y = modelo.jogador2.y = ALTURA_CAMPO * 0.5;
    modelo.bola.x = LARGURA_CAMPO * 0.5;
    modelo.bola.y = ALTURA_CAMPO * 0.5;
...
void Timer(int value)
{
  ...
  // testa colisÃ£o paredes
  if(modelo.bola.y >= ALTURA_CAMPO || modelo.bola.y <= 0){
      modelo.bola.direccao *= -1;
  }
  // testa colisÃ£o raquete 1
  if(cos(modelo.bola.direccao) < 0){
    if((modelo.bola.x <= (modelo.jogador1.x + LARGURA_RAQUETES)) && (modelo.bola.x >= (modelo.jogador1.x - LARGURA_RAQUETES)) && (modelo.bola.y <= (modelo.jogador1.y + modelo.alturaRaquetes * 0.5) && modelo.bola.y >= (modelo.jogador1.y - modelo.alturaRaquetes * 0.5))){
        GLfloat diff = modelo.bola.y - modelo.jogador1.y;
        GLfloat incr = RAD(diff) * 0.5;
        modelo.bola.direccao = M_PI - (modelo.bola.direccao - incr);
    }
  }else{
    // testa colisÃ£o raquete 2
    if((modelo.bola.x >= (modelo.jogador2.x - LARGURA_RAQUETES))&& (modelo.bola.x <= (modelo.jogador2.x + LARGURA_RAQUETES))  && (modelo.bola.y <= (modelo.jogador2.y + modelo.alturaRaquetes * 0.5) && modelo.bola.y >= (modelo.jogador2.y - modelo.alturaRaquetes * 0.5))){
        GLfloat diff = modelo.bola.y - modelo.jogador2.y;
        GLfloat incr = RAD(diff) * 0.5;
        modelo.bola.direccao = M_PI - (modelo.bola.direccao + incr);
    }
  }
...
void TimerTeclas(int value)
{
  ...
  if(estado.teclas.a && (modelo.jogador1.y - ALTURA_RAQUETES * 0.5) > 0)
  {
      modelo.jogador1.y -= modelo.velocidadeRaquetes;
  }
  if(estado.teclas.q && (modelo.jogador1.y + ALTURA_RAQUETES * 0.5) < ALTURA_CAMPO)
  {
      modelo.jogador1.y += modelo.velocidadeRaquetes;
  }
  if(estado.teclas.l && (modelo.jogador2.y - ALTURA_RAQUETES * 0.5) > 0)
  {
      modelo.jogador2.y -= modelo.velocidadeRaquetes;
  }
  if(estado.teclas.p && (modelo.jogador2.y + ALTURA_RAQUETES * 0.5) < ALTURA_CAMPO)
  {
      modelo.jogador2.y += modelo.velocidadeRaquetes;
  }

  // Â´nÃ£o necessita redesenhar o ecra, Ã© feito no Timer de animaÃ§Ã£o 
}
...
void menu(int opcao)
{
  ...
  switch (opcao) {
      ...
      case 9 :
            ...
            if(modelo.parado)
            {
              glutChangeToMenuEntry(1,"ComeÃ§ar",9);
              glutPostRedisplay();
            }
            else
            {
...
void cria_menu()
{
  ...
  // funÃ§oes de manipulaÃ§Ã£o de menus
...
/* DEFINIR MENUS */
  estado.menu_vel_bola_id = glutCreateMenu(menu); // regista funÃ§Ã£o que trata do menu
  glutAddMenuEntry("Diminuir", 1);
  glutAddMenuEntry("Aumentar", 2);

  estado.menu_tam_bola_id = glutCreateMenu(menu); // regista funÃ§Ã£o que trata do menu
  glutAddMenuEntry("Diminuir", 3);
  glutAddMenuEntry("Aumentar", 4);

  estado.menu_vel_raq_id = glutCreateMenu(menu); // regista funÃ§Ã£o que trata do menu
  glutAddMenuEntry("Diminuir", 5);
  glutAddMenuEntry("Aumentar", 6);

  estado.menu_tam_raq_id = glutCreateMenu(menu); // regista funÃ§Ã£o que trata do menu
  glutAddMenuEntry("Diminuir", 7);
  glutAddMenuEntry("Aumentar", 8);

  estado.menu_id=glutCreateMenu(menu); // regista funÃ§Ã£o que trata do menu
  ...
  glutAttachMenu(GLUT_RIGHT_BUTTON); // Agarrar menu actual ao botÃ£o
